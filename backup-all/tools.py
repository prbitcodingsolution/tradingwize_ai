...
Return ONLY valid JSON, no additional text."""
    ...
    def _check_and_backfill_missing_data(company_data: CompanyData, stock_symbol: str) -> CompanyData:
        """
        Check for missing fields in CompanyData and backfill them using Tavily search
        """
        import json
        import json
        import os
        ...
        search_domains = ["nseindia.com", "bseindia.com", "screener.in", "groww.in"]
...
{{
  ...
  "another_field": value
                        ...
                        response = client.chat.completions.create(
                            model="openai/gpt-3.5-turbo",  # Changed from gpt-oss-20b to gpt-3.5-turbo for better accuracy
                            messages=[
...
4. Ensure the ticker is currently active and traded"""
    ...
    def get_realtime_data(stock_symbol: str) -> CompanyData:
        ...
        # Safely get CEO name from company officers
        ceo = 'N/A'
        if info.get('companyOfficers') and len(info.get('companyOfficers', [])) > 0:
            ceo = safe_get(info.get('companyOfficers')[0], 'name', 'N/A', str)
    ...
    def format_data_for_report(company_data: CompanyData) -> str:
        ...
        promoter = fmt_pct(company_data.market_data.promoter_holding * 100) if company_data.market_data.promoter_holding else 'N/A'
        fii = fmt_pct(company_data.market_data.fii_holding * 100) if company_data.market_data.fii_holding else 'N/A'
        dii = fmt_pct(company_data.market_data.dii_holding * 100) if company_data.market_data.dii_holding else 'N/A'
...
Do NOT start with "Expert Opinion:" or any heading - just provide the analysis directly."""
    ...
    def generate_shark_tank_pitch(company_data: CompanyData) -> str:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="openai/gpt-oss-120b",  # Use the same model as configured in model_config.py
                messages=[
                    {"role": "system", "content": "You are a CEO/CFO presenting to Shark Tank investors. Generate a compelling, conversational pitch that flows naturally without any structural elements, numbered points, or section headers. Write in first person as the company, speaking directly to the Sharks."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,  # Slightly creative but consistent
                max_tokens=2000,  # Enough for a comprehensive pitch
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    def generate_shark_tank_pitch_prompt(company_data: CompanyData) -> str:
        ...
        prompt = f"""
You are the CEO/CFO of {company_data.name}. Generate a complete Shark Tank-style investment pitch presentation.
...
GENERATE A COMPLETE 14-POINT SHARK TANK PITCH:

Structure your pitch EXACTLY as follows (use conversational narrative, NOT bullet points):

🦈 SHARK TANK PITCH PRESENTATION
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. FOUNDER-STYLE INTRODUCTION
Start with: "Hello Sharks! I am CEO of [Company Name], and I'm here today to pitch my stock as a compelling investment opportunity..."
Explain what sector you're in and why you're exciting.

2. COMPANY SNAPSHOT
Give an overview: ticker symbol, market cap, industry position.
Use conversational language: "Let me give you a quick overview of who we are..."

3. BUSINESS OVERVIEW
Explain what the company does, revenue streams, market presence.
"Our business is built on a solid foundation..."

4. FINANCIAL PERFORMANCE
Present the numbers conversationally: "Now, let's talk numbers - because that's what really matters to you, Sharks."
Discuss revenue, profit, margins, cash flow, PE ratio, debt.
Explain what these numbers mean, not just list them.

5. STOCK INFORMATION & MARKET DATA
Current price, market cap, trading volume, liquidity.
"Our shares are currently trading at..."

6. PRICE PERFORMANCE
Historical performance, resilience, market recognition.
"Our stock has demonstrated resilience..."

7. COMPETITOR LANDSCAPE
Who you compete with and what sets you apart.
"In our industry, we compete with [competitors]..."
...
"I believe in transparency, so let me address the risks head-on..."

11. FUTURE GROWTH PLAN
Strategic roadmap, innovation plans.
"Looking ahead, Sharks, we have a clear roadmap for growth..."

12. SUMMARY FOR SHARKS
Bring it all together - conversational wrap-up.
"Sharks, let me bring this all together for you..."
End with: "So, Sharks, are you ready to invest in a company that combines stability with growth potential?"

13. VOICE PITCH SCRIPT
Three 20-second scripts:
- 20-Second Intro
- 20-Second Financial Summary  
- 20-Second Closing Pitch

14. Q&A MODE - READY FOR YOUR QUESTIONS
List the types of questions you're ready to answer.
"Sharks, I'm now ready to answer any questions you have..."

CRITICAL REQUIREMENTS:
- Write in FIRST PERSON as the company
- Use CONVERSATIONAL NARRATIVE style (like you're speaking to the Sharks)
- DO NOT ADD ANY NUMBERED POINTS (like 1., 2., 3., etc.)
- DO NOT ADD ANY SECTION TITLES OR HEADINGS (like "FOUNDER-STYLE INTRODUCTION", "COMPANY SNAPSHOT", etc.)
- DO NOT ADD ANY SEPARATOR LINES (━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━)
- NO bullet points - just continuous flowing text
- Include ALL actual numbers from the data
- Be persuasive but realistic
- Make it engaging and natural
- Each topic should flow naturally into the next without any breaks or formatting
- RESPOND WITH TEXT ONLY - no structural elements, just the narrative

Generate the COMPLETE pitch now as continuous narrative:
"""
