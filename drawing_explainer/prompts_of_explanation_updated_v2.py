# Main per-question prompt for the drawing-explainer LLM — v2.1 snapshot.
# Kept in lock-step with `_PER_QUESTION_SYSTEM` in llm_explainer.py.
#
# This revision adds the four fixes from `remaining_issues_spec.md`:
#   • Issue 6  — expanded per-profile tone rules (Section 12) with checkable
#                constraints (≤12 words/sentence on advance, etc.) instead of
#                vague adjectives.
#   • Issue 8  — `best_case` / `worst_case` schema fields with deterministic
#                sourcing rules quoting `trade_facts.rr_planned` /
#                `take_profit` / `stop_loss` / `stop_distance_pct` verbatim,
#                plus conditional risk-budget warnings at the 1% / 3% bands.
#   • Issue 10 — `INPUTS YOU RECEIVE` block promoted to Section 2 so input
#                declarations sit next to the rules that consume them. All
#                subsequent sections renumbered 2–16 → 3–17.
#   • New     — `score_breakdown.scoring_note` declared as AUDITOR_METADATA
#                (model ignores) in Section 9, plus a pre-flight checklist
#                item. The decision is recorded as a code comment above the
#                live `_PER_QUESTION_SYSTEM` in llm_explainer.py.
#
# When iterating on prompt wording: edit the live copy in llm_explainer.py
# first, then sync this file (or vice-versa) so they stay byte-identical.
